...
class _TitleRowState extends State<TitleRow> {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      ...
      if (order.status == 'Placed') Row(
        children: [
          CustomButtonSecondary(text: "Cancel", pressed: (){
            context.read<OrderDetailsProvider>().updateOrderStatus("Cancelled");
          }),
          CustomButton(text: "Accept", pressed: (){
            context.read<OrderDetailsProvider>().updateOrderStatus("Confirmed");
          }),
        ],
      )
